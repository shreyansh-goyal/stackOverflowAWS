...
//@access   PUBLIC
router.get("/" , (req,res)=>{
    Question.find().sort({date: 'desc'}).then(
...
router.post("/", passport.authenticate("jwt",{session:false}) , (req,res)=>{
    ...
    newQuestion.save().then(
        question =>{console.log("hello I am shreyansh");res.json(question)}
...
router.post("/answers/:id" ,passport.authenticate("jwt",{session:false}) , (req,res)=>{
    Question.findById(req.params.id).then(
        question=>{
            const newAnswer = {
                user:req.user.id,
                name:req.body.name,
            ...
            ).catch(err =>console.log("unable to push answer to db",err))
        }
